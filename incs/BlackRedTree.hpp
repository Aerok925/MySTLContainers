//
// Created by John Nappa on 6/1/22.
//

#ifndef CONTAINERS_BLACKREDTREE_HPP
#define CONTAINERS_BLACKREDTREE_HPP
...
//			_back = _node_alloc.allocate(1);
//			_node_alloc.construct(_back, Node<Value>());
//			_back->is_black = true;
//			_back->_left = _nil;
//			_back->_right = _nil;
		}
		...
		reverse_iterator rbegin(){
			return reverse_iterator(tree_max(_root));
		}

		const_reverse_iterator rbegin() const {
			return const_reverse_iterator(tree_max(_root));
		}

		reverse_iterator rend(){
			return reverse_iterator(tree_min(_root));
		}

		const_reverse_iterator rend() const {
			return const_reverse_iterator(tree_min(_root));
		}
		...
		ft::pair<iterator, bool>  insert(value_type const &_value){
			node_pointer find_val = search(_value, _root);
			if (find_val)
				return ft::pair<iterator, bool>(iterator(find_val), false);
			node_pointer new_node = _node_alloc.allocate(1);
			_node_alloc.construct(new_node, Node<value_type>(create_value(_value)));
			new_node->_left = _nil;
...
//			node_pointer tmp = tree_min(_root);
//			_back->_parent = tmp;
			return res;
		}
...
//			max_of_tree = tree_min(_root);
//			_back->_parent = max_of_tree;
			return (iterator(new_node));
		}
		...
		void erase(iterator pos){
			node_pointer y = pos.node_p(), x, for_free = y;
...
//			new_node = tree_min(_root);
//			new_node->_left = _back;
//			_back->_parent = new_node;
		}
		...
		RedBlackTree(const RedBlackTree& other) : _compare(other._compare), _root(NULL){
			*this = other;
		}

		template<class InputIt>
		RedBlackTree(typename ft::enable_if< !ft::is_integral<InputIt>::value, InputIt >::type first, InputIt last,
					 const value_compare& comp, const allocator_type& alloc = allocator_type()):	_alloc(alloc), _node_alloc(node_allocator()), _compare(comp) {
			init_nil_head();
			_root = _header;
			for (; first != last; ++first)
				insert(*first);
...
//			_node_alloc.deallocate(_back, 1);
...
//_______________________________________________________________________________________________________________//
		...
		node_pointer found(const value_type &x) const{
			node_pointer tmp;
			tmp = _root;
			while (tmp != _nil){
				if (x.first == tmp->_value->first)
					return tmp;
				if (_compare(x, *tmp->_value))
					tmp = tmp->_left;
				else
					tmp = tmp->_right;
			}
	...
	bool operator<( RedBlackTree<Content, Compare, Alloc> &lhs,  RedBlackTree<Content, Compare, Alloc>& rhs) {
	...
	template<class Content, class Compare, class Alloc>
	void swap( RedBlackTree<Content, Compare, Alloc>& lhs,  RedBlackTree<Content, Compare, Alloc> & rhs) {
		lhs.swap(rhs);
	};
	
}

#endif //CONTAINERS_BLACKREDTREE_HPP
